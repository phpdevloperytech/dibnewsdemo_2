...
(function ($) {
    ...
    $.HSCore.components.HSChartPie = {
        ...
        initCircles: function () {
            var lastItem = this.pageCollection.last(), lastId = 0, self = this;
            if (lastItem.length) {
                lastId = +lastItem.attr('id').substring(lastItem.attr('id').lastIndexOf('-') + 1);
            }
            this.collection.each(function (i, el) {
                var $this = $(el), id = 'hs-pie-' + (lastId + (i + 1)), value = 0;
                $this.attr('id', id);
            ...
            if (self.appearCollectionIds.length)
                self._initAppear();
        },
        /**
        ...
        getCircleAPI: function (index) {
            if (index && $.isNumeric(index) && this.pageCollection.eq(index).length)
