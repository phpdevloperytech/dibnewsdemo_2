...
(function dropdown($) {
    ...
    $.HSCore.components.HSDropdown = {
        ...
        init: function (collection, config) {
            ...
            $(document).on('click touchstart', 'body', function (e) {
                if (e.target.id == self._baseConfig.dropdownOpenedElement)
                    return;
                if ($(e.target).closest('#' + self._baseConfig.dropdownOpenedElement).length)
                    return;
        ...
        _bindEvents: function ($invoker, eventType, delay) {
            ...
            if (eventType === 'hover' && !_isTouch()) {
                $invoker.on('mouseenter.HSDropdown', function () {
                    var $invoker = $(this), HSDropdown = $invoker.data('HSDropdown');
                    if (!HSDropdown)
                        return;
                    if (HSDropdown.dropdownTimeOut)
                        clearTimeout(HSDropdown.dropdownTimeOut);
                    HSDropdown.show();
                ...
                $invoker.on('click.HSDropdown', function (e) {
                    var $curInvoker = $(this), $dropdownNotHasInnerDropdowns = $('[data-dropdown-target].active:not(.target-of-invoker-has-dropdowns)'), $dropdownHasInnerDropdown = $('[data-dropdown-target].active.target-of-invoker-has-dropdowns');
                    self._baseConfig.dropdownOpenedElement = $curInvoker.data('HSDropdown').target[0].id;
                    if (!$curInvoker.data('HSDropdown'))
                        return;
                ...
                if (Boolean($invoker.data('dropdown-target-is-menu'))) {
                    var $target = $($invoker.data('dropdown-target')), $targetItems = $target.children();
    ...
    function AbstractDropdown(element, config) {
        ...
        this.smartPosition = function (target) {
            ...
            var $w = $(window), styles = getComputedStyle(target.get(0)), direction = Math.abs(parseInt(styles.left, 10)) < 40 ? 'left' : 'right', targetOuterGeometry = target.offset();
            // horizontal axis
            if (direction === 'right') {
                if (!target.data('baseDirection'))
    ...
    function DropdownSimple(element, config) {
        if (!AbstractDropdown.call(this, element, config))
            return;
