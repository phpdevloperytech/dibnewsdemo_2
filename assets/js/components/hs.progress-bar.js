    ...
    $.HSCore.components.HSProgressBar = {
        ...
        HorizontalProgressBar: function (element, config) {
            ...
            if (this.config.useProgressElement) {
                ...
                this.element.data('value', this.indicator.length ? Math.round(this.indicator.outerWidth() / this.element.outerWidth() * 100) : 0);
        ...
        extendHorizontalProgressBar: function () {
            ...
            this.HorizontalProgressBar.prototype.update = function (value) {
                var self = this;
                if (this.config.useProgressElement) {
                    var fps = (this.config['time'] / this.config['fps']), iterationValue = parseInt(value / fps, 10), counter = 0, self = this;
                    if (iterationValue == 0)
                        iterationValue = 1;
                    ...
                    if (self.config.moveElementSelector && self.config['moveElementTo'] == 'end') {
                        ...
                        if (mIterationValue == 0)
                            mIterationValue = 1;
                        var mCounterId = setInterval(function () {
                            self.moveSubElement(mCounter += mIterationValue);
                            if (self.moveElement.length)
                                self.config.onMoveElementChange.call(self.moveElement, mCounter += mIterationValue);
                            if (mCounter > 100) {
                                clearInterval(mCounterId);
                                self.moveSubElement(100);
                                if (self.moveElement.length)
                                    self.config.afterMoveElementUpdate.call(self.moveElement);
                            }
                        }, fps);
                    }
                    this.element.data('intervalId', setInterval(function () {
                        ...
                        if (self.config.moveElementSelector && self.config['moveElementTo'] == 'currentPosition')
